...
# Change [2]: Import threading; fire opponent.prefetch_analysis() in background
#             the moment an opponent message arrives, not at the start of our turn.
...
def on_message(ws, message):
    ...
    # ── OPPONENT MESSAGE ──────────────────────────────────
        ...
        if msg and sender != MY_TEAM_NAME:
            ...
            # [Change 2] Fire Groq analysis immediately in a background thread.
            # By the time the server switches turn to us, the analysis is done.
            if topic and my_stance:
                threading.Thread(
                    target=opponent.prefetch_analysis,
                    args=(topic, my_stance),
                    daemon=True
                ).start()
                print("[main] 🔄 Groq prefetch started in background")
...
def take_turn(ws):
    ...
    # [Change 2] get_context_for_brain now returns prefetched analysis instantly
    # if background thread already completed, or falls back to sync call.
    opponent_ctx = opponent.get_context_for_brain(
        topic=topic,
        our_stance=my_stance,
        run_llm=len(opponent.history) > 0
    )
